# i.MX51 depth16 rendering

The rasterizer supports single-sample depth16 draws with finite viewport-transformed vertex depths in [0,1], including the Navigation clear with `RB_DEPTHCONTROL=0x7E`. It applies existing triangle coverage and scissoring, executes the pixel shader, compares depth, and stages color and depth writes together. All eight comparison functions and the depth write enable are honored. Disabling depth testing suppresses depth writes. A zero color mask does not suppress a depth write. Shader discard suppresses both color and depth writes.
...
Fragment depth converts as `min(floor(z_window * 65536), 65535)`, retaining double precision through viewport transformation, interpolation, and conversion. The Ford guest already programs normal depth scale/offset with a factor of 65535/65536. Applying another factor of 65535 to that transformed value would be incorrect.

Window depth uses screen-space barycentric interpolation at the pixel center, before the separate perspective correction of shader varyings. Clipped triangles derive varying depth from their generated vertices. Exactly constant transformed depth retains the original triangle's value, avoiding clipping-roundoff changes to the established constant-depth conversion. This follows the interpolation model required by Khronos GLES 2.0.25 section 3.5.1; hardware interpolation precision and rounding are not established by the constant-depth capture.
...
Out-of-range incoming depth, 24/8 depth-stencil, stencil operations, MSAA, overlapping active attachments, and additional fragment exports remain explicit failures. The early-depth bit is accepted, but the host implementation evaluates the supported fragment shader before testing depth. Physical addressing and preservation-copy formats require further evidence before those guards can be relaxed.
...
- [Khronos GLES 2.0.25](https://registry.khronos.org/OpenGL/specs/es/2.0/es_full_spec_2.0.pdf), section 3.5.1, printed page 58: window depth is the linear barycentric sum of vertex window depths; sections 2.13 and 2.13.1 define clipping and generated vertices.
